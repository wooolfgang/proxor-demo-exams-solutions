...
public class SpreadSheet extends JFrame {
    ...
    // limits the formula to 8 characters
    public String formatDisplay(String formula) {
        Double exp = Double.parseDouble(formula);
        int formatter = 7;
        String formatted = null; 
        
        do {
          formatted = String.format("%." + formatter + "g", exp);  
          formatter--;
        } while (formatted.length() != 8 && formatter != 0);
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        if (formula.length() > 0 && formula.charAt(0) == '=') {
            ...
            if (formula.length() > 8 && isDouble(formula) ) {
                String formattedText = formatDisplay(formula);
                cellsTF[r][c].setText(formattedText);
            } else {
    ...
    public void cellChanged(JTextField cell) {
        ...
        // do not change formula when it's length > 8
        if ((f.length() <= 0 || f.charAt(0) != '=') && !(f.length() > 8)) {
