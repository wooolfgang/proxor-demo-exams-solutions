...
public class SpreadSheet extends JFrame {
    ...
    private class Cell {
        ...
        public void setPreciseValue(String val) {
            preciseValue = val;
        }
    }
    ...
    public String evaluateToken(String tok, int depth) {
        if (tok.length() >= 2 && tok.charAt(0) >= 'A' && tok.charAt(0) < (char) ('A' + maxCols)) {
            ...
            if (row >= 0 && row < maxRows) {
                ...
                if (cells[row][col].bottom)
                    return null;
                return cells[row][col].preciseValue != null ? 
                       cells[row][col].preciseValue : cellsTF[row][col].getText();
            }
        }
        return null;
    }
    ...
    protected String add(String x, String y) throws NumberFormatException {
        return Double.toString(Double.parseDouble(x.trim()) + Double.parseDouble(y.trim()));
    }
    ...
    public String parseFormula(StringTokenizer tokens, int depth) throws NumberFormatException {
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        if (formula.length() > 0 && formula.charAt(0) == '=') {
            ...
            if (formula.length() > 8) {
                    String newFormula = limitFormula(formula); 
                    cellsTF[r][c].setText(newFormula);
                    cells[r][c].setPreciseValue(formula);
            } else {
    ...
    public String limitFormula(String formula) {
        try {
            ...
            if ((exp >= 0 && (int) exp > 999) || newFormula.length() > 8) {
                newFormula = String.format("%.3g", exp);
                if (newFormula.length() != 8) {
                    newFormula = String.format("%.6g", exp);
                }
            }  
            return newFormula;
        } catch (Exception e) {     
    ...
    public void cellChanged(JTextField cell) {
        ...
        if ((f.length() <= 0 || f.charAt(0) != '=') && !(f.length() > 8 && Double.parseDouble(f) >= 0)) {
    ...
    public void addComponentsToPane(final Container pane) {
        ...
        spreadSheetGrid.setPreferredSize(
                new Dimension((int) (cellSize.getWidth() * maxCols), (int) (cellSize.getHeight() * maxRows)));
        ...
        // Add column headings
        for (int c = 0; c < maxCols; c++) {
            JTextArea label = new JTextArea(Character.toString((char) ('A' + c)));
