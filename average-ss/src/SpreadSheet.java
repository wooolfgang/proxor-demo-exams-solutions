...
public class SpreadSheet extends JFrame {
    ...
    // parse the formula if it has an AVG syntax
    public String parseAvg(StringTokenizer tokens, int depth) {
        if (tokens.hasMoreTokens() && tokens.nextToken().equals("(")) {
           ...
           if (c1.charAt(0) == c2.charAt(0) || c1.charAt(1) == c2.charAt(1)) {
               ...
               for (char i = lowerCell; i <= higherCell; i++) {
                   String val = null;
                   if (sameCellIndex == 0) val = evaluateToken("" + c1.charAt(sameCellIndex) + i, depth);
                   else val = evaluateToken("" + i + c1.charAt(sameCellIndex), depth);
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if (tok.equals("AVG")) tok = parseAvg(tokens, depth);
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if (tok3.equals("AVG")) tok3 = parseAvg(tokens, depth);
