...
public class SpreadSheet extends JFrame implements ActionListener {
    ...
    File file; // the file associated with spreadsheet, if any
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if (cmd.equals("Open")) {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
				...
				// note that the File Open operation is implemented in FileIO.java
                if (!FileIO.open(this, file)) {
                	JOptionPane.showMessageDialog(null,
                			"Error opening spreadsheet", "File IO Error",
                			JOptionPane.WARNING_MESSAGE);
                }
            }
        } else if (cmd.equals("Save") && file != null) {
            // note that the File SaveAs operation is implemented in FileIO.java
    		if (!FileIO.saveAs(this, file)) {
    			JOptionPane.showMessageDialog(null, 
    					"Spreadsheet not saved", "File IO Error",
    					JOptionPane.WARNING_MESSAGE);
    		}
    	} else if (cmd.equals("Save") || cmd.equals("Save As...")) {
    		final JFileChooser fileChooser = new JFileChooser();
    		int result = fileChooser.showSaveDialog(this);
            if (result == JFileChooser.APPROVE_OPTION) {
            	file = fileChooser.getSelectedFile();
				// note that the File Save operation is implemented in 
				// FileIO.java
            	FileIO.saveAs(this, file);
            }
    	}
    }
    ...
    private static void createAndShowGUI() {
        ...
        frame.menuBar = new JMenuBar();
        frame.fileMenu = new JMenu("File");
        frame.openItem = new JMenuItem("Open", KeyEvent.VK_O);
        //frame.openItem.setMnemonic('O');
        frame.openItem.addActionListener(frame);
        frame.fileMenu.add(frame.openItem);
        frame.saveItem = new JMenuItem("Save");
        frame.saveItem.setMnemonic('S');
        frame.saveItem.addActionListener(frame);
        frame.fileMenu.add(frame.saveItem);
        frame.saveAsItem = new JMenuItem("Save As...");
        frame.saveAsItem.addActionListener(frame);
        frame.fileMenu.add(frame.saveAsItem);
        frame.menuBar.add(frame.fileMenu);
